...
| published | 公開日時 |
| updated | 更新日時 |
| featured | 任意・true/false。トップページ「TOPニュース」欄に表示したい記事を人が手動で選ぶためのフラグ
  （2026-07-13追加）。summarize.pyはデフォルトfalseで生成するため、目立たせたい記事があれば手動でtrueに
  変更する。ただしfeatured=trueでもsource_posted_atが24時間を過ぎると自動的にTOPから外れる
  （2026-07-14変更、下記「TOPページ掲載条件」参照） |
| source_type | influencer / official。記事化のきっかけがインフルエンサー発信か公式発表単独かの区別
  （2026-07-13追加）。summarize.pyは常に"influencer"を自動設定する（2026-07-14。news_raw.jsonの
  検知元がすべてx_sources.json記載アカウントのため）。TOPニュース・改悪情報欄の表示条件として使用 |
| source_account | きっかけとなったXアカウントのhandle（例："haiji_doctor"）。x_sources.json掲載アカウントを
  想定。summarize.pyがnews_raw.jsonのaccount_handleから自動設定する（2026-07-13追加、2026-07-14で自動化） |
| source_posted_at | 元Xツイートの投稿日時（ISO 8601、news_raw.jsonのcreated_atを引き継ぐ）。
  TOPニュース・改悪情報欄の「直近24時間以内」判定に使用（2026-07-14追加） |

## TOPページ掲載条件（2026-07-14変更）
トップページの「TOPニュース」「お得商品」「改悪情報」の3セクションは、以下の条件を満たす項目のみを
build.pyが自動抽出して表示する（該当なしの場合は「準備中です」等のプレースホルダー表示）。
- source_type=="influencer"（またはdeals.jsonの場合はsource_accountが設定されていること）
- source_posted_at（元Xツイートの投稿時刻）が、build.py実行時点から直近24時間以内であること
- TOPニュースのみ追加でfeatured=trueが必要（該当が複数ある場合はsource_posted_atが最新のものを採用）

この条件は/deals/ページ・/category/kaiaku/ページ・/category/campaign/ページの「最新号の内容」転載欄
（_latest_digest_html）にも共通して適用される（同じ関数を再利用しているため）。ただしカテゴリページ下部の
「関連ニュース一覧」は従来通り全期間のアーカイブとして時間フィルタなしで表示する。

【運用上の注意】この「24時間以内」判定はbuild.py実行時点の時刻を基準に静的HTMLを生成する方式のため、
サイトの表示を実際に鮮度良く保つには、収集（collect_news.py）だけでなく裏取り・summarize.py実行・
build.py再実行・GitHubアップロード（デプロイ）までを日次で回す必要がある。
...
| source_account | 紹介元インフルエンサーのXアカウントhandle（2026-07-13追加。x_sources.json掲載アカウントを想定）。
  2026-07-14よりTOPページ「お得商品」欄への掲載条件として必須扱いに変更（未設定の項目はTOPに出ない） |
| source_posted_at | 元Xツイートの投稿日時（ISO 8601、news_raw.jsonのcreated_atを転記）。TOPページ「お得商品」欄の
  「直近24時間以内」判定に使用（2026-07-14追加。必須。手動でnews_raw.jsonの該当項目のcreated_atをコピーする） |
| updated | 最終更新日・取得日 |

deals.jsonへの掲載条件はTOPページ共通ルール（本ファイル上部「TOPページ掲載条件」参照）に従う。
source_accountとsource_posted_atの両方が設定され、かつsource_posted_atが直近24時間以内でなければ
TOPページ・/deals/ページには表示されない（データ自体はdeals.jsonに残るが非表示になるだけ）。
...
3a. candidate_type="news"の項目: summarize.py がverified項目をClaude APIで中立要約し、
    news.jsonに追記。source_type="influencer"・source_account・source_posted_at
    （news_raw.jsonのaccount_handle/created_atを転記）は自動設定される（2026-07-14）。
    featuredはfalseで生成されるため、TOPニュースに出したい記事は生成後に手動でtrueへ変更する。
...
3b. candidate_type="deal"の項目: 自動化スクリプトは未整備のため、人手でdeals.jsonの
    ...
    source_posted_at）を設定して追記する。source_accountはnews_raw.jsonのaccount_handle、
    source_posted_atはcreated_atをそのまま転記する（この2項目が無いとTOPページに表示されない）。
    urlは official_source_url をベースに、可能なら自社アフィリエイトリンクへ差し替える
    （is_affiliate=trueにしPR表記）。反映後はnews_raw.json側の該当項目のstatusをpublishedに
    更新し、二重登録を避ける
